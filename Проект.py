...
def db_init():
    try:
        # создание двух таблиц в БД(страны и диапазоны IP)
...
# возвращает страну(для отображдения наиминования)
# возвращает краткий код страны(для поиска флага)
def get_country(ip):
    try:
        cur = connection.cursor()
...
def get_flag(country_code):
    return fr'flags-medium/{country_code.lower()}.png'
...
try:
    ...
    class IP(QWidget):
        ...
        def initUI(self):
            ...
            # кнопка для узнования своего IP
            ...
            # кнопка для расчёта страны по IP
            ...
            # отображение стартовой картинки
            self.pixmap = QPixmap(r"flags-medium\ip_start.png")
        ...
        # функция для получениея IP пользователя
        def my_ip(self):
